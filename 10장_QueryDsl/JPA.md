...
<img src="./images/10.4_1.PNG" width="800" height="500"/>
...
JPAQueryFactory queryFactory;  //필드 레벨로 선언
...
```
- JPAQueryFactory queryFactory = new JPAQueryFactory(em);를 필드로 뺄 수 있다.
- 동시 멀티쓰레드로 EntityManger에 동시에 접근해도 프레임워크에서 주입하는 엔티티 자체가 멀티쓰레드에 문제없게 설계가 되어있다. (동시성문제X) : 권장
...
- 쿼리 타입의 기본 인스턴스를 사용하면 import static을 활용해서 코드를 더 간결하게 작성할 수 있다. (권장)
- JPQL은 문자로 작성되서 문법 오류가 생길 시 런타임(사용자가 실제 실행) 오류가 발생하지만 Q타입의 QueryDSL은 컴파일 시 오류가 발생해서 바로 오류를 잡을 수 있다.

![ConnectionMaker](./images/10.4_5.PNG)    

- 쿼리 타입을 기본 인스턴스로 보관해서 사용하면 QMember.member가 아닌 member로 사용할 수 있게 된다. 
- 같은 테이블을 조인해야할 경우에만 별칭을 주면된다.
  
> 참고 : application.yml 파일의 jpa: 부분에서 use_sql_comments:true를 추가하면 QueryDSL 문법을 JPQL로 변환되는것도 콘솔에 찍히게 해준다.
(QueryDSL은 결국 JPQL로 변환해서 SQL을 날리는거다. )
...
- where(a,b,c)은 where a and b and c로 변환된다. (null은 무시한다. 동적쿼리시 좋다.)
> 참고 : select().from()을 selectFrom()로 사용가능
