...
3. mappedBy 속성은 양방향 매핑일때 사용하는데 Member 객체의 team필드가 연관관계의 주인이니 team을 이름값으로 값을 줬다.
...
## 5.4 연관관계의 주인
1. 단방향일때는 외래키를 한 엔티티만 소유해서 참조를 하면되지만 양방향일때는 서로 참조가 가능하니 외래키를 관리해야할 엔티티를 설정해줘야한다.
